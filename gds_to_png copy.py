...
def export_png(cells, interactive=False):
    ...
    print(f"Layout bounds: x({min_x}, {max_x}) y({min_y}, {max_y})")

    block_width = ceil((max_x - min_x))
    block_height = ceil((max_y - min_y))

    print(f"Block size: {block_width}um x {block_height}um")
    ...
    print(f"Image size: {image.shape[1]} x {image.shape[0]} pixels in {steps_x} x {steps_y} tiles")
    ...
    print(f"Offset applied: {offset[0]} x {offset[1]} pixels")
    ...
    for x_base in range(steps_x):
        for y_base in range(steps_y):
            ...
            print(f"Processing tile shape {tile.shape} at base ({x_base}, {y_base})")
    ...
    print(f"Cropped image size: {image.shape[1]} x {image.shape[0]} pixels")
...
if __name__ == '__main__':
    ...
    for gds_file in image_directory.glob('*.gds'): # [Path('imaging/wrapped_snn_network-poly.gds')]:
        ...
        if str(gds_file) == "imaging/wrapped_function_generator_poly.gds":
