    ...
    for psi in try_angles:
        for name in names:
            ...
            # now template match the reference onto the image so we can determine the offset
                ...
                # max_loc is in (x, y) format (cols, rows) or (width, height)
                # image.shape is in (rows, cols) format (height, width)
                ...
                cv2.imwrite(f'imaging/{name}_{layer}_corrected_1.png', corrected_image)
                blended = cv2.addWeighted(corrected_image, 1.0, composite_overlay, 0.5, 0)
                cv2.imwrite(f'imaging/{name}_{layer}_blended_1.png', blended)
                ...
                cv2.imwrite(f'imaging/{name}_{layer}_corrected_1.png', corrected_image_corrected)
            ...
            cv2.imwrite(f'imaging/{name}_{layer}_aligned_1.png', label_blended)
            ...
            with open(f'imaging/{name}_{layer}{psi}_1.pkl', 'wb') as f:
                pickle.dump(entry, f)
            meta = {
            ...
            with open(f'imaging/{name}_{layer}{psi}_1.meta', 'wb') as f:
                pickle.dump(meta, f)
